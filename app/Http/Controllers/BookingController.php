...
class BookingController extends Controller
{
    ...
    public function storeCheckIn(StoreCheckInBookingRequest $request): JsonResponse
    {
        ...
        // Get parking slot
        $parkingSlot = ParkingSlot::findOrFail($request->parking_slot_id);
        
        // Verify the slot belongs to the specified parking location
        if ($parkingSlot->parking_location_id !== $parkingLocation->id) {
            return response()->json([
                'message' => 'The selected slot does not belong to the specified parking location.',
            ], 422);
        }
        ...
        if (!$parkingSlot->is_active) {
            return response()->json([
                'message' => 'The selected parking slot is not active.',
            ], 422);
        }
        ...
        // Verify the slot vehicle type matches the vehicle
        $vehicleType = $vehicle->type === '2-wheeler' ? '2-wheeler' : '4-wheeler';
        if ($parkingSlot->vehicle_type !== $vehicleType) {
            return response()->json([
                'message' => 'The selected parking slot is not compatible with your vehicle type.',
            ], 422);
        }
        ...
        try {
            ...
            return response()->json([
                ...
                'booking' => new BookingResource($booking->load(['vehicle', 'parkingLocation', 'parkingSlot'])),
            ], 201);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function storeAdvance(StoreAdvanceBookingRequest $request): JsonResponse
    {
        ...
        $slotIsBooked = ParkingBooking::where('parking_slot_id', $parkingSlot->id)
            ->where('status', '!=', 'cancelled')
            ->where(function ($query) use ($checkInDateTime, $durationHours) {
                $checkOutDateTime = (clone $checkInDateTime)->addHours($durationHours);
                
                // Check if there's any overlap with existing bookings
                $query->where(function ($q) use ($checkInDateTime, $checkOutDateTime) {
                    $q->where(function ($inner) use ($checkInDateTime, $checkOutDateTime) {
                        // New booking starts during an existing booking
                        $inner->where('check_in_time', '<=', $checkInDateTime)
                              ->where('check_out_time', '>=', $checkInDateTime);
                    })->orWhere(function ($inner) use ($checkInDateTime, $checkOutDateTime) {
                        // New booking ends during an existing booking
                        $inner->where('check_in_time', '<=', $checkOutDateTime)
                              ->where('check_out_time', '>=', $checkOutDateTime);
                    })->orWhere(function ($inner) use ($checkInDateTime, $checkOutDateTime) {
        ...
        if ($slotIsBooked) {
            return response()->json([
                'message' => 'The selected slot is already booked for the requested time period.',
            ], 422);
        }
        ...
        try {
            ...
            $booking = ParkingBooking::create([
                ...
                'check_out_time' => $checkInDateTime->copy()->addHours($durationHours),
